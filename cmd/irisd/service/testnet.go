...
// TestnetCmd initialises files required to start iris testnet
func testnetCmd(ctx *server.Context, cdc *codec.Codec) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "create-testnet",
		Short: "Initialize files for a Iris testnet",
...
`,
			...
			for i := 0; i < totalValidators; i++ {
				...
				if i < numValidators {
					...
					validators[i] = hmTypes.NewValidator(
						...
						10000,
						newPubKey,
						hmTypes.BytesToIrisAddress(valPubKeys[i].Address().Bytes()),
					)
				...
				WriteDefaultIrisConfig(filepath.Join(config.RootDir, "config/iris-config.toml"), helper.GetDefaultIrisConfig())
			}
	...
	cmd.Flags().String(flagNodeDaemonHome, "irisd",
