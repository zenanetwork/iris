...
func main() {
	...
	executor := cli.PrepareMainCmd(rootCmd, "HD", os.ExpandEnv("/var/lib/Iris"))
...
func exportCmd(ctx *server.Context, _ *codec.Codec) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(_ *cobra.Command, _ []string) error {
			...
			genesisTimes := map[string]string{
				"Iris-137":   "2020-05-30T04:28:03.177054Z",    // mainnet
				"Iris-80001": "2020-06-04T10:47:20.806665Z",    // mumbai
				"Iris-80002": "2023-11-06T06:41:35.410487141Z", // amoy
				"devnet":     "2025-01-01T00:00:00.000000000Z", // local devnet
			}

			genesisTime, ok := genesisTimes[chainID]
			if !ok {
				panic("invalid chain-id, it must be one of: Iris-137 (mainnet), Iris-80001 (mumbai), Iris-80002 (amoy), devnet (local)")
			}
	...
	cmd.Flags().String(client.FlagChainID, "devnet", "Genesis file chain-id, it can be "+
		"Iris-137 (for mainnet), "+
		"Iris-80001 (for mumbai), "+
		"Iris-80002 (for amoy), "+
...
func generateMarshalledAppState(happ *app.IrisApp, chainID, genesisTime string, maxNextGenesisItems int, w io.Writer) error {
