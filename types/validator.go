...
// the IrisAddress field is generated using Address i.e. [20]byte
func NewValidator(
	id ValidatorID,
	startEpoch uint64,
	endEpoch uint64,
...
// to sort it we compare the values of the Signer(IrisAddress i.e. [20]byte)
...
func (v *Validator) ValidateBasic() bool {
	...
	if bytes.Equal(v.Signer.Bytes(), IrisAddress(common.Address{}).Bytes()) {
		return false
	}
