...
func handleMsgCheckpoint(ctx sdk.Context, msg types.MsgCheckpoint, k Keeper, _ helper.IContractCaller) sdk.Result {
	...
	logger.Debug("Validator account root hash generated", "accountRootHash", hmTypes.BytesToIrisHash(accountRoot).String())
	...
	if !bytes.Equal(accountRoot, msg.AccountRootHash.Bytes()) {
		logger.Error(
			...
			"hash", hmTypes.BytesToIrisHash(accountRoot).String(),
