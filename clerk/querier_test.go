...
func (suite *QuerierTestSuite) TestHandleQueryRecord() {
	...
	hAddr := hmTypes.BytesToIrisAddress([]byte("some-address"))
	hHash := hmTypes.BytesToIrisHash([]byte("some-address"))
...
func (suite *QuerierTestSuite) TestHandleQueryRecordSequence() {
	...
	index = simulation.RandIntBetween(r1, 0, 100)
	logIndex = uint64(index)
	txHash = hmTypes.HexToIrisHash("1234")
	...
	logIndex = uint64(10)
	txHash = hmTypes.HexToIrisHash("12345")
