...
func (suite *SideHandlerTestSuite) TestSideHandleMsgEventRecord() {
	...
	t.Run("NoReceipt", func(t *testing.T) {
		...
		txHash := hmTypes.HexToIrisHash("no receipt hash")
	...
	t.Run("NoLog", func(t *testing.T) {
		...
		txHash := hmTypes.HexToIrisHash("no log hash")
...
func (suite *SideHandlerTestSuite) TestPostHandleMsgEventRecord() {
	...
	t.Run("Replay", func(t *testing.T) {
		...
		logIndex := r.Uint64()
		blockNumber := r.Uint64()
		txHash := hmTypes.HexToIrisHash("Replay hash")
		...
		msg := types.NewMsgEventRecord(
			...
			blockNumber,
			id,
			hmTypes.BytesToIrisAddress(addr2.Bytes()),
			make([]byte, 0),
			suite.chainID,
		)
