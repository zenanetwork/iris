...
import (
	...
	"github.com/zenanetwork/iris/bridge/setu/processor"
...
func StartBridgeWithCtx(shutdownCtx context.Context) error {
	...
	for loop {
		...
		case <-time.After(waitDuration):
			if !util.IsCatchingUp(cliCtx) {
				...
				logger.Info("Waiting for iris to be synced")
			}
		}
	}
	...
	g.Go(func() error {
		...
		logger.Info("Received stop signal - Stopping all iris bridge services")
		for _, service := range services {
			srv := service
			if srv.IsRunning() {
