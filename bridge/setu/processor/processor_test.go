...
func TestBroadcastWhenTxInMempool(t *testing.T) {
	...
	helper.InitIrisConfig(os.ExpandEnv("$HOME/.irisd"))
	...
	defaultMessage := clerkTypes.MsgEventRecord{
		...
		TxHash:          hmTypes.BytesToIrisHash([]byte("0xa86a2f8f30d5fab1bb858e572ecf3f24d691276f833f06fc90a745cea20f4fcb")),
		LogIndex:        71,
		BlockNumber:     14475337,
		ContractAddress: hmTypes.BytesToIrisAddress([]byte("0x401f6c983ea34274ec46f84d70b31c151321188b")),
		Data:            []byte{},
		ID:              1897091,
		ChainID:         "15001",
	}
	...
	defaultMessage2.TxHash = hmTypes.BytesToIrisHash([]byte("0x8a83aa78a400fe959b44ccf70d926c967af4e451ba630a849b2e1dedc7e30c07"))
	...
	for index, tx := range testData { //nolint
		...
		t.Run(string(rune(index)), func(t *testing.T) {
			...
			if !inMempool {
				...
				_, err := _txBroadcaster.BroadcastToIris(tx, nil)
				assert.Empty(t, err, "Error broadcasting tx to iris", err)
			} else {
