...
func (cp *CheckpointProcessor) RegisterTasks() {
	...
	if err := cp.queueConnector.Server.RegisterTask("sendCheckpointToIris", cp.sendCheckpointToIris); err != nil {
		cp.Logger.Error("RegisterTasks | sendCheckpointToIris", "error", err)
	}
	...
	if err := cp.queueConnector.Server.RegisterTask("sendCheckpointAckToIris", cp.sendCheckpointAckToIris); err != nil {
		cp.Logger.Error("RegisterTasks | sendCheckpointAckToIris", "error", err)
	}
}
...
// sendCheckpointToIris - handles headerblock from maticchain
// 1. check if i am the proposer for next checkpoint
// 2. check if checkpoint has to be proposed for given headerblock
// 3. if so, propose checkpoint to iris.
func (cp *CheckpointProcessor) sendCheckpointToIris(headerBlockStr string) (err error) {
	...
	if isProposer {
		...
		if err := cp.createAndSendCheckpointToIris(checkpointContext, start, end); err != nil {
			cp.Logger.Error("Error sending checkpoint to iris", "error", err)
			return err
		}
	} else {
...
// sendCheckpointToRootchain - handles checkpoint confirmation event from iris.
...
func (cp *CheckpointProcessor) sendCheckpointToRootchain(eventBytes string, blockHeight int64) error {
	...
	if err := jsoniter.ConfigFastest.Unmarshal([]byte(eventBytes), &event); err != nil {
		cp.Logger.Error("Error unmarshalling event from iris", "error", err)
		return err
	}
...
// sendCheckpointAckToIris - handles checkpointAck event from rootchain
// 1. create and broadcast checkpointAck msg to iris.
func (cp *CheckpointProcessor) sendCheckpointAckToIris(eventName string, checkpointAckStr string) error {
	// fetch checkpoint context
	checkpointContext, err := cp.getCheckpointContext()
	if err != nil {
		return err
	...
	if err = helper.UnpackLog(cp.rootchainAbi, event, eventName, &log); err != nil {
		...
		cp.Logger.Info(
			"✅ Received task to send checkpoint-ack to iris",
			"event", eventName,
			"start", event.Start,
			"end", event.End,
			"reward", event.Reward,
			...
			"txHash", hmTypes.BytesToIrisHash(log.TxHash.Bytes()),
		...
		msg := checkpointTypes.NewMsgCheckpointAck(
			...
			hmTypes.BytesToIrisAddress(event.Proposer.Bytes()),
			event.Start.Uint64(),
			event.End.Uint64(),
			event.Root,
			hmTypes.BytesToIrisHash(log.TxHash.Bytes()),
			uint64(log.Index),
		)

		// return broadcast to iris
		txRes, err := cp.txBroadcaster.BroadcastToIris(msg, event)
		if err != nil {
			cp.Logger.Error("Error while broadcasting checkpoint-ack to iris", "error", err)
			return err
		}

		if txRes.Code != uint32(sdk.CodeOK) {
			cp.Logger.Error("checkpoint-ack tx failed on iris", "txHash", txRes.TxHash, "code", txRes.Code)
...
// 3. Send NoAck to iris if required.
...
// sendCheckpointToIris - creates checkpoint msg and broadcasts to iris
func (cp *CheckpointProcessor) createAndSendCheckpointToIris(checkpointContext *CheckpointContext, start uint64, end uint64) error {
	cp.Logger.Debug("Initiating checkpoint to Iris", "start", start, "end", end)
	...
	cp.Logger.Info("Root hash calculated", "rootHash", hmTypes.BytesToIrisHash(root))

	var accountRootHash hmTypes.IrisHash
	//Get DividendAccountRoot from IrisServer
	if accountRootHash, err = cp.fetchDividendAccountRoot(); err != nil {
		cp.Logger.Info("Error while fetching initial account root hash from IrisServer", "err", err)
		return err
	}
	...
	cp.Logger.Info("✅ Creating and broadcasting new checkpoint",
		"start", start,
		"end", end,
		"root", hmTypes.BytesToIrisHash(root),
	...
	// return broadcast to iris
	txRes, err := cp.txBroadcaster.BroadcastToIris(msg, nil)
	if err != nil {
		cp.Logger.Error("Error while broadcasting checkpoint to iris", "error", err)
		return err
	}

	if txRes.Code != uint32(sdk.CodeOK) {
		cp.Logger.Error("Checkpoint tx failed on iris", "txHash", txRes.TxHash, "code", txRes.Code)
...
func (cp *CheckpointProcessor) createAndSendCheckpointToRootchain(checkpointContext *CheckpointContext, start uint64, end uint64, height int64, txHash []byte) error {
	cp.Logger.Info("Preparing checkpoint to be pushed on chain", "height", height, "txHash", hmTypes.BytesToIrisHash(txHash), "start", start, "end", end)
	// proof
...
func (cp *CheckpointProcessor) fetchDividendAccountRoot() (accountroothash hmTypes.IrisHash, err error) {
	cp.Logger.Info("Sending Rest call to Get Dividend AccountRootHash")

	response, err := helper.FetchFromAPI(cp.cliCtx, helper.GetIrisServerEndpoint(util.DividendAccountRootURL))
	if err != nil {
		cp.Logger.Error("Error Fetching accountroothash from IrisServer ", "error", err)
	...
	if err = jsoniter.ConfigFastest.Unmarshal(response.Result, &accountroothash); err != nil {
		cp.Logger.Error("Error unmarshalling accountroothash received from Iris Server", "error", err)
		return accountroothash, err
	}
...
func (cp *CheckpointProcessor) getLastNoAckTime() uint64 {
	response, err := helper.FetchFromAPI(cp.cliCtx, helper.GetIrisServerEndpoint(util.LastNoAckURL))
	if err != nil {
...
// proposeCheckpointNoAck - sends Checkpoint NoAck to iris
func (cp *CheckpointProcessor) proposeCheckpointNoAck() (err error) {
	...
	txRes, err := cp.txBroadcaster.BroadcastToIris(msg, nil)
	if err != nil {
		cp.Logger.Error("Error while broadcasting checkpoint-no-ack to iris", "msg", msg, "error", err)
		return err
	}

	if txRes.Code != uint32(sdk.CodeOK) {
		cp.Logger.Error("Checkpoint No-Ack tx failed on iris", "txHash", txRes.TxHash, "code", txRes.Code)
