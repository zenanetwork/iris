...
const (
	...
	chainManagerParamsUrl      = dummyIrisServerUrl + "/chainmanager/params"
	...
	getAccountUrl      = dummyIrisServerUrl + "/auth/accounts/9FB29AAC15B9A4B7F17C3385939B007540F4D791"
	getAccountUrl2     = dummyIrisServerUrl + "/auth/accounts/3F17F1962B36E491B30A40B2405849E597BA5FB5"
	getAccountResponse = `
	{
	"height": "11384869",
	"result": {
	...
	isOldTxUrl      = dummyIrisServerUrl + "/clerk/isoldtx?logindex=0&txhash=0x6d428739815d7c84cf89db055158861b089e0fd649676a0243a2a2d204c1d854"
	isOldTxResponse = `
	{
		"height": "11384858",
		"result": false
	...
	checkpointCountUrl      = dummyIrisServerUrl + "/checkpoints/count"
	...
	getValidatorSetUrl      = dummyIrisServerUrl + "/staking/validator-set"
...
func BenchmarkSendStateSyncedToIris(b *testing.B) {
	b.ReportAllocs()
	b.ResetTimer()
	b.StopTimer()
	...
	for i := 0; i < b.N; i++ {
		func() {
			...
			if err = cp.sendStateSyncedToIris("StateSynced", dlb.String()); err != nil {
				b.Fatal(err)
			}

			b.StopTimer()

			b.Log("StateSynced sent to iris successfully")
		}()
	}
}
	...
	for i := 0; i < b.N; i++ {
		func() {
			...
			rcl.SendTaskWithDelay(
				"sendStateSyncedToIris", "StateSynced",
				logs.Bytes(), ts[i], nil)
			b.StopTimer()
