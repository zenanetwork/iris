...
func GetFromAddress(cliCtx context.CLIContext) types.IrisAddress {
	...
	if !fromAddress.Empty() {
		return types.AccAddressToIrisAddress(fromAddress)
	}

	return types.BytesToIrisAddress(GetAddress())
}
...
func PrepareTxBuilder(cliCtx context.CLIContext, txBldr authTypes.TxBuilder) (authTypes.TxBuilder, error) {
	...
	// get iris address
	fhAddress := types.BytesToIrisAddress(from)
...
func populateAccountFromState(txBldr authTypes.TxBuilder, cliCtx context.CLIContext, addr []byte) (authTypes.TxBuilder, error) {
	...
	// key
	key := hmTypes.BytesToIrisAddress(addr)
...
func GetIrisServerEndpoint(endpoint string) string {
	u, _ := url.Parse(GetConfig().IrisServerURL)
