...
func (suite *KeeperTestSuite) TestValidator() {
	...
	require.Equal(t, types.HexToIrisAddress(mappedSignerAddress.Hex()), validators[0].Signer, "Signer address doesn't match")
}
...
func (suite *KeeperTestSuite) TestUpdateSigner() {
	...
	newSigner := types.HexToIrisAddress(newPubKey.Address().String())
	...
	require.Equal(t, newSigner, types.HexToIrisAddress(signerAddress.Hex()), "Validator ID should be mapped to Updated Signer Address")
