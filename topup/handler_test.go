...
func (suite *HandlerTestSuite) TestHandleMsgTopup() {
	...
	txHash := hmTypes.BytesToIrisHash([]byte("topup hash"))
	logIndex := r1.Uint64()
	...
	t.Run("Success", func(t *testing.T) {
		msg := types.NewMsgTopup(
			hmTypes.BytesToIrisAddress(addr.Bytes()),
			hmTypes.BytesToIrisAddress(addr.Bytes()),
			fee,
			txHash,
			logIndex,
...
func (suite *HandlerTestSuite) TestHandleMsgWithdrawFee() {
	...
	t.Run("FullAmount", func(t *testing.T) {
		...
		acc1 := app.AccountKeeper.NewAccountWithAddress(ctx, hmTypes.AccAddressToIrisAddress(addr))
		...
		acc1 = app.AccountKeeper.GetAccount(ctx, hmTypes.AccAddressToIrisAddress(addr))
