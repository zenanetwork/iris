...
func (suite *SideHandlerTestSuite) TestSideHandleMsgTopup() {
	...
	t.Run("Success", func(t *testing.T) {
		...
		txHash := hmTypes.HexToIrisHash("success hash")
...
func (suite *SideHandlerTestSuite) TestPostHandleMsgTopup() {
	...
	txHash := hmTypes.HexToIrisHash("hash")
	...
	t.Run("NoResult", func(t *testing.T) {
		...
		acc1 := app.AccountKeeper.GetAccount(ctx, hmTypes.AccAddressToIrisAddress(addr1))
		require.Nil(t, acc1)
	})
	...
	t.Run("WithProposer", func(t *testing.T) {
		logIndex := r.Uint64()
		blockNumber := r.Uint64()
		txHash := hmTypes.HexToIrisHash("hash with proposer")
		...
		// topup msg
		msg := types.NewMsgTopup(
			hmTypes.BytesToIrisAddress(addr2.Bytes()), // different proposer
			hmTypes.BytesToIrisAddress(addr3.Bytes()),
			coins.AmountOf(authTypes.FeeToken),
			txHash,
			logIndex,
			blockNumber,
		...
		acc2 := app.AccountKeeper.GetAccount(ctx, hmTypes.AccAddressToIrisAddress(addr2))
		require.NotNil(t, acc2)
		require.False(t, acc2.GetCoins().Empty())

		// account coins should be empty
		acc3 := app.AccountKeeper.GetAccount(ctx, hmTypes.AccAddressToIrisAddress(addr3))
	...
	t.Run("Replay", func(t *testing.T) {
		logIndex := r.Uint64()
		blockNumber := r.Uint64()
		txHash := hmTypes.HexToIrisHash("hash replay")
