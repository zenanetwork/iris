...
balanceInit=$(docker exec zena0 bash -c "zena attach /var/lib/zena/data/zena.ipc -exec 'Math.round(web3.fromWei(eth.getBalance(eth.accounts[0])))'")
...
do
    ...
    balance=$(docker exec zena0 bash -c "zena attach /var/lib/zena/data/zena.ipc -exec 'Math.round(web3.fromWei(eth.getBalance(eth.accounts[0])))'")

    if ! [[ "$balance" =~ ^[0-9]+$ ]]; then
        echo "Something is wrong! Can't find the balance of first account in zena network."
        exit 1
    fi
