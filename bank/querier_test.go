...
func (suite *QuerierTestSuite) TestQueryBalance() {
	...
	req.Data = cdc.MustMarshalJSON(types.NewQueryBalanceParams(hmTypes.BytesToIrisAddress([]byte(""))))
	res, sdkErr = querier(ctx, path, req)
	...
	req.Data = cdc.MustMarshalJSON(types.NewQueryBalanceParams(hmTypes.AccAddressToIrisAddress(addr)))
	res, sdkErr = querier(ctx, path, req)
	require.NoError(t, sdkErr)
	require.NotNil(t, res)
	...
	// set account
	acc1 := happ.AccountKeeper.NewAccountWithAddress(ctx, hmTypes.AccAddressToIrisAddress(addr))
...

	{
		...
		store.Set(authTypes.AddressStoreKey(hmTypes.AccAddressToIrisAddress(addr)), []byte(""))
		require.Panics(t, func() {
