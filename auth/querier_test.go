...
func (suite *QuerierTestSuite) TestQueryAccount() {
	...
	req.Data = cdc.MustMarshalJSON(types.NewQueryAccountParams(hmTypes.BytesToIrisAddress([]byte(""))))
	res, err = querier(ctx, path, req)
	require.Error(t, err)
	require.Nil(t, res)

	_, _, addr := sdkAuth.KeyTestPubAddr()
	req.Data = cdc.MustMarshalJSON(types.NewQueryAccountParams(hmTypes.AccAddressToIrisAddress(addr)))
	res, err = querier(ctx, path, req)
	require.Error(t, err)
	require.Nil(t, res)

	happ.AccountKeeper.SetAccount(ctx, happ.AccountKeeper.NewAccountWithAddress(ctx, hmTypes.AccAddressToIrisAddress(addr)))
	res, err = querier(ctx, path, req)
...

	{
		...
		store.Set(types.AddressStoreKey(hmTypes.AccAddressToIrisAddress(addr)), []byte(""))
		require.Panics(t, func() {
