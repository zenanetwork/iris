...
func (ak AccountKeeper) NewAccountWithAddress(ctx sdk.Context, addr hmTypes.IrisAddress) types.Account {
	acc := ak.proto()
...
func (ak AccountKeeper) GetAccount(ctx sdk.Context, addr hmTypes.IrisAddress) types.Account {
...
func (ak AccountKeeper) GetPubKey(ctx sdk.Context, addr hmTypes.IrisAddress) (crypto.PubKey, sdk.Error) {
...
func (ak AccountKeeper) GetSequence(ctx sdk.Context, addr hmTypes.IrisAddress) (uint64, sdk.Error) {
	acc := ak.GetAccount(ctx, addr)
	if acc == nil {
...
func (ak AccountKeeper) GetBlockProposer(ctx sdk.Context) (hmTypes.IrisAddress, bool) {
	store := ctx.KVStore(ak.key)
	if !store.Has(types.ProposerKey()) {
		return hmTypes.IrisAddress{}, false
	}

	bz := store.Get(types.ProposerKey())

	return hmTypes.BytesToIrisAddress(bz), true
}

// SetBlockProposer sets block proposer
func (ak AccountKeeper) SetBlockProposer(ctx sdk.Context, addr hmTypes.IrisAddress) {
	store := ctx.KVStore(ak.key)
