...
func (suite *AnteTestSuite) TestGasLimit() {
	...
	// set the accounts
	acc1 := happ.AccountKeeper.NewAccountWithAddress(ctx, hmTypes.AccAddressToIrisAddress(addr1))
...
func (suite *AnteTestSuite) TestCheckpointGasLimit() {
	...
	acc2 := happ.AccountKeeper.NewAccountWithAddress(ctx, hmTypes.AccAddressToIrisAddress(addr2))
...
func (suite *AnteTestSuite) TestAccountNumbersAtBlockHeightZero() {
	...
	// acc1 = happ.AccountKeeper.GetAccount(ctx, hmTypes.AccAddressToIrisAddress(addr1))
	// acc2 = happ.AccountKeeper.GetAccount(ctx, hmTypes.AccAddressToIrisAddress(addr2))
...
func (suite *AnteTestSuite) TestFees() {
	...
	require.True(sdk.IntEq(t, happ.AccountKeeper.GetAccount(ctx, hmTypes.AccAddressToIrisAddress(addr1)).GetCoins().AmountOf(authTypes.FeeToken), sdk.NewInt(149)))
	...
	require.True(sdk.IntEq(t, happ.AccountKeeper.GetAccount(ctx, hmTypes.AccAddressToIrisAddress(addr1)).GetCoins().AmountOf(authTypes.FeeToken), sdk.NewInt(0)))
