...
func (suite *KeeperTestSuite) TestAccountMapperGetSet() {
	...
	addr := hmTypes.BytesToIrisAddress([]byte("some-address"))
...
func (suite *KeeperTestSuite) TestAccountMapperRemoveAccount() {
	...
	addr1 := hmTypes.BytesToIrisAddress([]byte("addr1"))
	addr2 := hmTypes.BytesToIrisAddress([]byte("addr2"))
...
func (suite *KeeperTestSuite) TestGetSequence() {
	...
	_, err = app.AccountKeeper.GetSequence(ctx, hmTypes.BytesToIrisAddress([]byte("addr3")))
	require.Error(t, err)
}
...
func (suite *KeeperTestSuite) TestGetPubKey() {
	...
	addr1 := hmTypes.AccAddressToIrisAddress(addr)
	acc1 := app.AccountKeeper.NewAccountWithAddress(ctx, addr1)
	...
	_, err = app.AccountKeeper.GetPubKey(ctx, hmTypes.BytesToIrisAddress([]byte("addr3")))
	require.Error(t, err)
}
...
func (suite *KeeperTestSuite) TestIterateAccounts() {
	...
	for i := 0; i < newAccounts; i++ {
		addr := hmTypes.BytesToIrisAddress([]byte(fmt.Sprintf("address-%v", i)))
		acc := app.AccountKeeper.NewAccountWithAddress(ctx, addr)
...
func (suite *KeeperTestSuite) TestProposer() {
	...
	require.False(t, ok)
	require.Equal(t, hmTypes.ZeroIrisAddress.Bytes(), proposer.Bytes())
